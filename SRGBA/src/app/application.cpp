...
void Application::draw_menu_bar() {
    ...
    if (ImGui::BeginMenu("Emulation")) {
        ...
        if (ImGui::MenuItem("Reset", nullptr, false, emulator_.has_rom())) {
            emulator_.reset();
            status_message_ = "ROM reset (scaffold state)";
...
void Application::draw_landing_page() {
    ...
    ImGui::TextDisabled("Game Boy Advance emulator - foundation build");
    ImGui::Spacing();
    ImGui::TextWrapped(
        "Open a legally obtained .gba ROM to inspect its cartridge header and exercise the "
        "desktop frontend. CPU execution is the next implementation milestone.");
    ImGui::Spacing();
...
void Application::draw_game_view() {
    ...
    ImGui::Separator();
    ImGui::TextDisabled(
        "Scaffold framebuffer - ARM7TDMI execution is not implemented yet (frame %llu)",
...
void Application::draw_about_window() {
    ...
    ImGui::TextUnformatted("SRGBA 0.1.0");
    ImGui::SetWindowFontScale(1.0F);
    ImGui::TextWrapped(
        "A clean-room Game Boy Advance emulator project built with C++20, SDL3, and Dear ImGui.");
    ImGui::Spacing();
    ImGui::TextDisabled("Current status: application and core scaffold");
...
void Application::load_rom(const std::filesystem::path& path) {
    ...
    status_message_ = "ROM loaded. CPU emulation is the next milestone.";
    status_is_error_ = false;
}
